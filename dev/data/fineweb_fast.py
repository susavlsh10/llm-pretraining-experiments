...
def init_worker(model_desc):
    """Initialize tokenizer in each worker process"""
    global tokenizer, encoding, model_type
    model_type = model_desc
    
    if model_desc == "llama-3":
        tokenizer = AutoTokenizer.from_pretrained("meta-llama/Meta-Llama-3.1-8B")
    elif model_desc == "gpt-2":
        encoding = tiktoken.get_encoding("gpt2")
    else:
        raise ValueError(f"unknown model {model_desc}")

def tokenize_document(doc):
    """Tokenizes a single document using the pre-initialized tokenizer"""
    global tokenizer, encoding, model_type
    ...
    if model_type == "llama-3":
        ...
        encode = lambda s: encoding.encode_ordinary(s)
...
with mp.Pool(nprocs, initializer=init_worker, initargs=(args.model_desc,)) as pool:
    shard_index = 0
